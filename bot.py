...
#  Проверка на ссылку сайта авто.ру
def islink(text):
...
def parse(url, chat_id):
    ...
    if html.status_code == 200:
        car_list = []
        pages = pc.get_pages_count(html.text) # количество страниц
        for page in range(1, pages + 1):
            # сообщение пользователю, что процесс пошел
            bot.send_message(chat_id, f'Парсинг страницы {page} из {pages} ...')
            # добавление авто с текущей страницы
...
def welcome(message):
    ...
    bot.send_sticker(chat_id, stick)  # отправка стикера
